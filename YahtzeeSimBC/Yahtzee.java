...
public class Yahtzee {
    public static final double VERSION = 202035.002;
    ...
    public boolean setScore( String type ) {
        int[] roll = new int[5];
    ...
    private void jokerScoring() {
        ...
        if ( upperScore[ val - 1 ] == null ) {
            ...
            // If the appropriate box is filled in you may score in 3K, 4K,FH,SS,LS,C
            
            // You may wish to choose different logic for this section
            if ( lowerScore[LARGESTRAIGHT] == null ) { lowerScore[LARGESTRAIGHT] = 40; } else
                // high score 4k
            if ( val >= 5 && lowerScore[FOURKIND] == null )  { lowerScore[FOURKIND] = 5*val; } else
                // high score 3k
            if ( val >= 5 && lowerScore[THREEKIND] == null ) { lowerScore[THREEKIND] = 5*val; } else
            if ( lowerScore[SMALLSTRAIGHT] == null ) { lowerScore[SMALLSTRAIGHT] = 30; } else
                // high score chance
            if ( val >= 5 && lowerScore[CHANCE] == null ) { lowerScore[CHANCE] = 5*val; } else
            if ( lowerScore[FULLHOUSE] == null ) { lowerScore[FULLHOUSE] = 25; } else
                // lower score 4k
            if ( lowerScore[FOURKIND] == null ) { lowerScore[FOURKIND] = 5*val; } else
                // lower score 3k
            if ( lowerScore[THREEKIND] == null ) { lowerScore[THREEKIND] = 5*val; } else
                // lower score chance
            if ( lowerScore[CHANCE] == null ) { lowerScore[CHANCE] = 5*val; } else {
                // If both the appropriate upper box and all lower boxes are filled you must
                // score a 0 in an upper box.
                for (int i = 0; i < 6; i++ ) {
